...
A Tauri 2, React 18, TypeScript, and Tailwind desktop shell for AgentUI.

This app lives in the `desktop/` workspace of the AgentUI monorepo. For normal setup and startup, run commands from the repository root.
...
- Rust Tauri command bridge that calls the Hermes API and management sidecar.
- Management-sidecar discovery for Hermes profiles, `MEMORY.md`, `USER.md`, and skills.
...
- Hermes management API: profile, memory, skill, status, and conversation reads come from the monorepo sidecar in `../sidecar`, defaulting to `http://127.0.0.1:8765/v1`.

Settings treats the selected profile API URL as the primary chat route, then falls back to the default local or remote API URL based on the connection mode. Profiles can store their own API URLs, matching Hermes' multi-profile pattern where each profile gateway runs on its own port. Conversation listing and detail are read through the management sidecar. The desktop app does not read local `state.db` history directly and does not keep a browser-side conversation cache; Hermes endpoints are the source of truth.
