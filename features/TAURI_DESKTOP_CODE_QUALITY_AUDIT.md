...
Start with a small PR that extracts only pure helpers from `useHermesChat.ts` and `ChatView.tsx`:

- `chatStreamMerging.ts`
- `chatHistory.ts`
- `toolEvents.ts`
- `shared/strings.ts`
- `shared/files.ts`

This should reduce the two riskiest chat files without changing React state ownership. It also creates a safer testable base for the later hook/component split.
