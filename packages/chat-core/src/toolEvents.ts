import type { ChatStreamToolEvent, CoreChatMessage, CoreHistoryToolCall } from "./types";

export function streamToolEventFromHistory(message: CoreChatMessage): ChatStreamToolEvent {
...
export function mergeStreamToolEvent(
  current: ChatStreamToolEvent[],
  next: ChatStreamToolEvent,
): ChatStreamToolEvent[] {
...
export function streamToolEventFromLegacyContent(content: string): ChatStreamToolEvent {
...
export function streamToolEventsFromMetadata(metadata: Record<string, unknown>, id: string): ChatStreamToolEvent[] {
...
export function toolStatusLabel(status: ChatStreamToolEvent["status"]) {
  if (status === "completed") return "Done";
  if (status === "error") return "Error";
  return "Running";
}

export function toolEventDetail(event: ChatStreamToolEvent) {
  const sections = [];
...
function historyToolName(message: CoreChatMessage, data: Record<string, unknown> | null) {
  if (message.toolName) return message.toolName;
  return legacyToolName(data);
}

function streamToolEventFromCall(toolCall: CoreHistoryToolCall, fallbackCallId: string): ChatStreamToolEvent {
...
function metadataToolCalls(metadata: Record<string, unknown>): CoreHistoryToolCall[] {
  const value = metadata.toolCalls || metadata.tool_calls;
  return Array.isArray(value)
    ? value.filter((item): item is CoreHistoryToolCall => Boolean(item && typeof item === "object" && !Array.isArray(item)))
    : [];
}
...
function historyToolStatus(data: Record<string, unknown> | null): ChatStreamToolEvent["status"] {
