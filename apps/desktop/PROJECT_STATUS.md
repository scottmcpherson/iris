...
Hermes remains the first runtime backend through the Iris Hermes Adapter. Hermes-specific filesystem reads, session discovery, Jobs API calls, model and slash-command discovery, and inbound message delivery are contained in the Core Hermes runtime adapter plus the Hermes-side `iris-platform` plugin.

## Implemented

- Desktop shell with sidebar navigation, chat workspace, settings/profile views, memory view, skills view, automations view, and agents view.
- Core-backed chat creation, message send, event streaming, cancellation, session list, and session detail.
